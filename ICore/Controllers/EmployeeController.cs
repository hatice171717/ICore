...
namespace A_ICore.Controllers
{
    ...
    public class EmployeeController : Controller
    {
        ...
        {
            var httpClient = new HttpClient();
            var responseMessage = await httpClient.GetAsync("https://localhost:7139/api/Default/" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        public async Task<IActionResult> EditEmployee(Class1 p)
        {
            ...
            var responseMessege = await httpClient.PutAsync("https://localhost:7139/api/Default", content);
        ...
        {
            var httpClient = new HttpClient();
            var responseMessage = await httpClient.DeleteAsync("https://localhost:7139/api/Default/" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
